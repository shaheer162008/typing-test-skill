...
export default function WordLessonPicker({ wordCount }: { wordCount: number }) {
  const [activeIndex, setActiveIndex] = useState(0);
  const { lessons: firestoreLessons, loading, error } = useFirestoreLessons(`words-${wordCount}`);
  const visibleLessons = firestoreLessons.map((item) => ({ id: item.id, title: item.title, label: `Lesson ${String(item.order).padStart(2, "0")}`, preview: item.text, description: item.focus ?? "Focused word practice." }));
  ...
  const move = (direction: number) => setActiveIndex((current) => (current + direction + visibleLessons.length) % visibleLessons.length);
        ...
        <div className="border border-black/20 p-5 sm:p-8">
          <div className="flex items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-1 text-xs text-black/50">{loading ? "Connecting to Firestore" : error ?? `${lesson?.label ?? "No lesson"} · ${wordCount} words`}</p>
            </div>
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-black text-primary"><Play className="h-4 w-4 fill-current" aria-hidden="true" /></div>
          </div>

          <div className="mt-7 border border-black/20 bg-[#f7f5ec] p-4 text-black sm:p-5">
            <p className="text-[10px] uppercase tracking-[0.18em] text-black/45">Passage preview</p>
          ...
          <div className="mt-5 grid gap-2 text-xs text-black/60 sm:grid-cols-3">
            {['Fixed word count', 'No countdown', 'Instant result'].map((item) => <span key={item} className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5" aria-hidden="true" />{item}</span>)}
          </div>
          ...
          <Link href={startHref} aria-disabled={!lesson} onClick={(event) => { if (!lesson) event.preventDefault(); }} className={`group mt-5 flex items-center justify-between bg-black px-5 py-4 text-sm font-semibold text-primary transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 focus-visible:ring-offset-primary ${lesson ? "" : "pointer-events-none opacity-50"}`}>
            Start {wordCount}-word test <ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" aria-hidden="true" />
          </Link>
          ...
          <div className="mt-5 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2" aria-label="Lesson selector">
              {visibleLessons.map((item, index) => <button key={item.label} type="button" onClick={() => setActiveIndex(index)} aria-label={`Show ${item.title}`} aria-pressed={index === activeIndex} className={`h-2 transition-all ${index === activeIndex ? "w-8 bg-black" : "w-2 bg-black/25"}`} />)}
            </div>
