...
const lessons = [
  { title: "Warm-up words", label: "Lesson 01", preview: "the and for with that this from have your", description: "Familiar words to settle your fingers into a clean rhythm." },
  { title: "Everyday flow", label: "Lesson 02", preview: "people create better habits through steady practice", description: "A balanced mix of common words for a natural benchmark." },
  { title: "Focus and control", label: "Lesson 03", preview: "calm hands make accurate movement feel automatic", description: "Build reliable control by slowing down and catching mistakes." },
];

export default function WordLessonPicker({ wordCount }: { wordCount: number }) {
  const [activeIndex, setActiveIndex] = useState(0);
  const lesson = lessons[activeIndex];
  const startHref = `/test?mode=words&wordCount=${wordCount}`;
        ...
        <div className="border border-black/20 p-5 sm:p-8">
          <div className="flex items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-1 text-xs text-black/50">{lesson.label} · {wordCount} words</p>
            </div>
