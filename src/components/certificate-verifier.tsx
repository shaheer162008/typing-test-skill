...
export default function CertificateVerifier() {
  ...
  const verify = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await response.json() as { valid: boolean; certificate?: CertificateRecord };
      setResult(data.valid ? data.certificate ?? null : null);
    } catch {
      setResult(null);
    } finally {
      setBusy(false);
      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
        ...
        <div>
          <form onSubmit={verify} className="flex flex-col gap-3">
            ...
            <button type="submit" disabled={busy} className="inline-flex min-h-12 items-center justify-center gap-2 bg-primary px-5 text-sm font-semibold text-black transition hover:bg-primary/85 disabled:opacity-60"><Search className="h-4 w-4" aria-hidden="true" /> {busy ? "Checking…" : "Verify certificate"}</button>
          </form>
          {searched && (result ? <div className="mt-5 border border-emerald-300/30 bg-emerald-300/[0.06] p-5" role="status"><div className="flex items-center gap-2 text-sm font-medium text-emerald-200"><CheckCircle2 className="h-5 w-5" aria-hidden="true" /> Certificate verified</div><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-xs uppercase tracking-[0.14em] text-primary/40">Issued to</dt><dd className="mt-1">{result.name ?? "Typing Test Skill learner"}</dd></div><div><dt className="text-xs uppercase tracking-[0.14em] text-primary/40">Issued by</dt><dd className="mt-1">Typing Test Skill</dd></div><div><dt className="text-xs uppercase tracking-[0.14em] text-primary/40">Result</dt><dd className="mt-1">{result.rawWpm} WPM · {result.accuracy}% accuracy</dd></div><div><dt className="text-xs uppercase tracking-[0.14em] text-primary/40">Issued</dt><dd className="mt-1">{new Date(result.issuedAt).toLocaleDateString()}</dd></div></dl></div> : <div className="mt-5 flex gap-3 border border-red-300/25 bg-red-300/[0.05] p-5 text-sm text-red-100" role="alert"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true" /><span>No verified certificate was found for that ID.</span></div>)}
        </div>
      </div>
    </section>
  );
